...
import { MdManageSearch, MdArrowBackIos, MdArrowForwardIos } from 'react-icons/md'
...
function ResultArchive() {
  ...
  const [filteredArchiveList, setFilteredArchiveList] = useState(archiveList);
  ...
  useEffect(() => {
    ...
    setFilteredArchiveList(filteredList);
    setCurrentPage(1); // Reset page number when search term changes
  }, [archiveList, searchTerm]);

  useEffect(() => {
    const indexOfLastItem = currentPage * itemsPerPage;
    const indexOfFirstItem = indexOfLastItem - itemsPerPage;
    const items = filteredArchiveList.slice(indexOfFirstItem, indexOfLastItem);
    setCurrentItems2(items);
  }, [currentPage, filteredArchiveList, itemsPerPage]);
  ...
  const handleSelectAllCheckboxChange = (event) => {
    if (event.target.checked) {
      const allIds = archiveList.map(item => item.id_arquivo);
  ...
  const handleActionsButtonClick = () => {
    ...
    setIdArquivo(selectedRows.join(','));
  ...
  const renderPageInfo = () => {
    const totalPages = Math.ceil(filteredArchiveList.length / itemsPerPage);
    return (
      <div>
  ...
  const handlePageChange = (e) => {
    ...
    if (!isNaN(page) && page > 0 && page <= Math.ceil(filteredArchiveList.length / itemsPerPage)) {
      setCurrentPage(page);
    }
  };
  ...
  const indexOfLastItem = currentPage * itemsPerPage;
  const indexOfFirstItem = indexOfLastItem - itemsPerPage;
  const currentItems = filteredArchiveList.slice(indexOfFirstItem, indexOfLastItem);
  ...
  const anonimizarExpressao = () => {
    ...
    const headers = {
      id_arquivos_sep_virgula: idArquivo,
  ...
  const moverArquivo = () => {
    ...
    api
      ...
      .then((resp) => {
        ...
        getArchiveList(jsonKey, jsonValue)
        setEnderecoDestino("");
        setSelectedRows([]);
              ...
              {Object.keys(expList[0]).map((key, index) => {
                ...
                if (value !== null && value !== '') {
                  return (
                    <tr key={index}>
                      <td>
                        <button onClick={() => getArchiveList(key, value)}><MdManageSearch size={18} /></button>
                      </td>
                      <td>{value}</td>
                    </tr>
                  );
        ...
        <div className="modal-container">
          <div className="modal-content">
            ...
            <div className="header-action">
              <div className="t1">
                <h1 className="card-title">Arquivos para a expressão: <span style={{ color: "#333", fontWeight: "normal" }}>{jsonValue}</span></h1>
            ...
            <div className="tabela-arquivos">
              <table>
                  ...
                  <tr>
                    <th>
                      <input
                        ...
                        checked={selectedRows.length === archiveList.length}
                      />
                    </th>
                ...
                <tbody>
                  {currentItems.map((row) => (
                    <tr key={row.id_arquivo}>
                      <td>
                        <input
                          type="checkbox"
                          onChange={(e) => handleRowCheckboxChange(e, row.id_arquivo)}
                          checked={selectedRows.includes(row.id_arquivo)}
                        />
                      </td>
              ...
              <div className="paginacao">
                <button onClick={handleActionsButtonClick}>Ações</button>
                <div>
                  ...
                  <input
                    type="number"
                    min="1"
                    max={Math.ceil(filteredArchiveList.length / itemsPerPage)}
                    value={currentPage}
                    onChange={handlePageChange}
                    style={{ minWidth: '2vw', width: "auto", border: "none", backgroundColor: "#eee" }}
                  />
                  de {Math.ceil(filteredArchiveList.length / itemsPerPage)}
                  &nbsp;
                  <button onClick={() => setCurrentPage(currentPage + 1)} disabled={currentPage === Math.ceil(filteredArchiveList.length / itemsPerPage)}><MdArrowForwardIos size={18} /></button>
                </div>
        ...
        isModalMoverOpen && (
          <div className="modal-container">
            ...
            <div className="modal-find-opcoes-tabela atualizar-pesquisa">
              ...
              <div>
                <label className="label-modal" htmlFor="select-maquina-destino">Insira o endereço de destino</label>
                <input
                  type="text"
        ...
        isModalPseudoAnonimizarOpen && (
          <div className="modal-container">
            ...
            <div className="modal-find-opcoes-tabela atualizar-pesquisa">
              ...
              <div className="botoes-modal">
                <button className="button" onClick={() => pseudoAnonimizarExpressao()}>Pseudo-anonimizar</button>
...

      {
        isModalReverterPseudoAnonimizarOpen && (
          <div className="modal-container">
            ...
            <div className="modal-find-opcoes-tabela atualizar-pesquisa">
              <div className="divider-modal">
                <h1>Reverter Pseudo-anonimizar Expressão</h1>
              </div>
              <div>
                <label className="label-modal" htmlFor="input-endereco-destino">Origem do Arquivo: </label>
                <p id="endereco-arquivo">{enderecoArquivo}</p>
              ...
              <div className="botoes-modal">
                <button className="button" onClick={() => reverterPseudoAnonimizarExpressao()}>Reverter Pseudo-anonimizar</button>
